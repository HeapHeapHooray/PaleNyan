...
- `--timeout-time <value>` (e.g., `--timeout-time 60` or `--timeout-time 90`):
  Patches the `timeout-time` property under settings in `spigot.yml` with the provided value right before starting the server. This works with or without `--run-only`.
