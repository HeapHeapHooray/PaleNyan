...
for world in "${DETECTED_WORLDS[@]}"; do
    ...
    # Map standard and custom worlds to their vanilla/namespaced dimension IDs for execute in
    execute_ns="minecraft:overworld"
    if [[ "$world" == "$MAIN_WORLD" ]]; then
        execute_ns="minecraft:overworld"
    elif [[ "$world" == *"nether"* ]]; then
        execute_ns="minecraft:the_nether"
    elif [[ "$world" == *"end"* ]]; then
        execute_ns="minecraft:the_end"
    else
        execute_ns="minecraft:$world"
    fi

    # Set world gamerules
    COMMANDS_CONTENT="${COMMANDS_CONTENT}  \"mvrule minecraft:random_tick_speed 0 $world\":\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    delay: 2\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    notify-on-exec: false\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}  \"mvrule minecraft:spawn_mobs false $world\":\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    delay: 2\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    notify-on-exec: false\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}  \"mvrule minecraft:spawn_monsters false $world\":\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    delay: 2\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    notify-on-exec: false\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}  \"mvrule minecraft:advance_time false $world\":\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    delay: 2\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    notify-on-exec: false\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}  \"mvrule minecraft:advance_weather false $world\":\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    delay: 2\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    notify-on-exec: false\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}  \"mvrule minecraft:show_death_messages false $world\":\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    delay: 2\n"
    COMMANDS_CONTENT="${COMMANDS_CONTENT}    notify-on-exec: false\n"
